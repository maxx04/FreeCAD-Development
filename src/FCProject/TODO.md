-[ ] Bennenungen zu Teilenahmen hinzufügen (z.B Latte )

-[ ] JSON Versionsprüfung kontrollieren

1. Halbzeuge:
1. TraceParts
2. AddOns FreeCAD
3. Eigene Halbzeuge 
Halbzeuge interessant seitens Kostenabschätzung.

1. Rückwärtserzeugung Halbzeuge soll möglich sein!

1. JSON als include in ProjectManager Reinziehen.

1. Materialänderung in Tree synchronisieren als .ShapeMatirial.Name

1. Profile mit dem Material Anlegen. Als stück vom Profil.

1. Beim Rohteil Material aus Profil Kopieren.

1. Erstellung part aus Sheet Metall prüfen.

1. B als Kaufteile hinzufügen.

1. Beim Part fragen nach Halbzeug, wenn ja integrieren.

1. Änderung: Projekt anlegen - Verzeichnis mit Projektname anlegen und dort json mit Projektname. Gemeinsame Verzeichnisse für  Profile, Materialien, Kaufteile finden.

1. Versionskontrolle hinzufügen.










